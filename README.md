...
- [Core Fraud Detection Capabilities](#-core-fraud-detection-capabilities)
- [Behavioral Profiling](#-behavioral-profiling)
- [Dynamic Fraud Risk Scoring](#-dynamic-fraud-risk-scoring)
- [Multi-Source Event Correlation](#-multi-source-event-correlation)
- [Cyber Analyst Portal](#-cyber-analyst-portal)
- [Interactive Money Flow Graph](#-interactive-money-flow-graph)
- [Audit-Ready Investigation Reports](#-audit-ready-investigation-reports)
- [Key Differentiators](#-key-differentiators)
- [System Architecture](#️-system-architecture)
- [Data Model](#️-data-model)
- [Technology Stack](#️-technology-stack)
- [Complete Detection Workflow](#-complete-detection-workflow)
...
**Example:** A fraudster may avoid detection by sending:

```
₹5,000 → ₹5,000 → ₹10,000 → ₹15,000
```

instead of a single suspicious transaction of:
...
Quantra Correlate combines multiple signal sources into one unified fraud intelligence layer:

```
Transactions + Sessions + Devices + IP Addresses + Geolocation
+ Behavior + Account Relationships + Security Events
        ↓
Unified Fraud Intelligence
```

The platform continuously correlates these signals to determine whether activity is:

| Status | Meaning |
|--------|---------|
| 🟢 Normal | No anomalies detected |
| 🟡 Suspicious | Minor deviation from baseline |
| 🟠 High Risk | Multiple correlated risk signals |
| 🔴 Fraudulent | Confirmed malicious activity |
...
```
Legitimate User → Active Session → Attacker Reuses Session
      → Different Device / IP / Location → ATO Detection
      → Real User Verification → Transaction Approved / Blocked
```

For high-risk transactions, the legitimate user can be prompted:
...
```
Multiple Login Attempts → Pattern Correlation
   → Suspicious IP / Device Detection
   → Credential Stuffing Identified → Account Protection
```
...
Fraudulent funds often move through multiple accounts to hide their original source. Quantra Correlate uses transaction relationships to identify suspicious money-flow networks.

**Example:**

```
Account A --₹50,000--> Account B --₹45,000--> Account C --₹40,000--> Account D
```

The platform visualizes these relationships through an **interactive money-flow graph**, helping investigators identify:
...
Every user has a normal behavioral pattern. Quantra Correlate compares current activity against historical behavior.

**Example:**

| | Amount |
|---|---|
| Historical Average | ₹5,000 |
| Current Activity | ₹5,000 → ₹7,000 → ₹10,000 → ₹15,000 → ₹20,000 |
...
```
Transaction Risk + Behavioral Deviation + Session Risk + Device Risk
+ IP Risk + Location Risk + Receiver Risk
        ↓
   Dynamic Risk Score
```
...
Instead of analyzing individual events independently — Transaction, Session, Device, IP, Location, Login, Receiver, Behavior — Quantra Correlate combines them into a unified investigation context.

**Example:**

```
Normal Transaction + New Device + New IP + Unusual Location
+ Abnormal Amount + New Receiver
        ↓
   HIGH-RISK BEHAVIOR
```
...
Analysts can investigate an account using:
...
```
User → Session → Device → IP → Transaction → Receiver → Risk Decision
```
...
| **Data Visualization** | Interactive transaction graphs, risk visualizations, behavioral analytics, timeline-based activity analysis |
| **Reporting** | PDF investigation reports |
